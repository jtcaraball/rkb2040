...
pub fn run() -> ! {
    ...
    if pins.vbus_detect.into_pull_up_input().is_high().unwrap() {
        ...
        let keys = init_primary_pins!(pins);
        let mut kb = PrimaryKB::new(keys, rx, sm);
        ...
        let keys = init_secondary_pins!(pins);
        let mut kb = SecondaryKB::new(keys, tx);
